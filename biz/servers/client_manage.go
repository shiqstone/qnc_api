...
func (manager *ClientManager) EventDisconnect(client *Client) {
	...
	if len(client.GroupList) > 0 {
		for _, groupName := range client.GroupList {
			SendMessage2Group(client.SystemId, sendUserId, groupName, "offline", errno.WS_OFFLINE_MESSAGE_CODE, "client offline", &data)
		}
	}
...
func (manager *ClientManager) GetByClientId(clientId string) (*Client, error) {
	...
	if client, ok := manager.ClientIdMap[clientId]; !ok {
		return nil, errors.New("client not existed")
	} else {
		return client, nil
	}
}
...
// 发送到本机分组
func (manager *ClientManager) SendMessage2LocalGroup(systemId, messageId, sendUserId, groupName string, msgType string, code int, msg string, data *string) {
	if len(groupName) > 0 {
...
// 发送给指定业务系统
func (manager *ClientManager) SendMessage2LocalSystem(systemId, messageId string, sendUserId string, msgType string, code int, msg string, data *string) {
	if len(systemId) > 0 {
...
func (manager *ClientManager) AddClient2LocalGroup(groupName string, client *Client, userId string, extend string) {
	...
	//发送系统通知
	SendMessage2Group(client.SystemId, sendUserId, groupName, "system", errno.WS_ONLINE_MESSAGE_CODE, "client online", &data)
}
